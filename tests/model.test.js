...
assert.deepEqual(first.chords[0], { id: 1, text: "Super + A", durationMs: 2000, shownAt: 0, fadingAt: 0 })
...
const holding = Model.advanceChords(live, 400, 300)
assert.equal(holding.changed, false)
assert.equal(holding.chords.length, 3)
const fading = Model.advanceChords(live, 700, 300)
assert.equal(fading.changed, true)
assert.deepEqual(fading.chords.map(function(chord) { return chord.id }), [1, 2, 3])
assert.equal(fading.chords[1].fadingAt, 700)
assert.equal(fading.chords[0].fadingAt || 0, 0)
const kept = Model.advanceChords(fading.chords, 900, 300)
assert.equal(kept.changed, false)
assert.equal(kept.chords.length, 3)
const dropped = Model.advanceChords(fading.chords, 1000, 300)
assert.equal(dropped.changed, true)
assert.deepEqual(dropped.chords.map(function(chord) { return chord.id }), [1, 3])
